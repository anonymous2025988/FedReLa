...
def plot_metrics(overall, many, medium, few, args, names=('overall', 'many', 'medium', 'few')):
    ...
    参数:
    overall (np.array): overall指标数组
    many (np.array): many指标数组
    medium (np.array): medium指标数组
    few (np.array): few指标数组
    names (tuple): 四个指标的名称，默认('overall', 'many', 'medium', 'few')
    """
    # 校验输入数组长度一致
...
if __name__ == '__main__':
    ...
    if len(load_switch) > 3:
        ...
        model = torch.load(load_dir + args.load +"model_" + "best" + ".pth", weights_only=False,map_location=torch.device('cpu')).to(args.device)
        g_head = torch.load(load_dir + args.load+"g_head_" + "best" + ".pth", weights_only=False,map_location=torch.device('cpu')).to(args.device)
        g_aux = torch.load(load_dir + args.load+"g_aux_" + "best" + ".pth",weights_only=False,map_location=torch.device('cpu')).to(args.device)
        for i in range(args.num_users):
            l_heads[i] = torch.load(load_dir + args.load+"l_head_" + str(i) + "best.pth",weights_only=False,map_location=torch.device('cpu')).to(args.device)
        w_glob = model.state_dict()
        w_locals = [copy.deepcopy(w_glob) for i in range(args.num_users)]
    else:
        rndstart = 0
    ...
    for rnd in tqdm(range(rndstart, args.rounds)):
        ...
        for client_id in idxs_users:
            ...
            w_local, g_aux_temp, l_heads[client_id], _ = local.update_weights_gaux(net=copy.deepcopy(model).to(args.device), 
