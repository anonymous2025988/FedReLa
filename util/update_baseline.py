...
class LocalUpdate(object):
    ...
    def relabel_local_data(self, g_head, net, net_id, args, relabel_thre, class_num=100, read=False, store=False, method='loge'):
        """
        Relabel local data based on prediction statistics and z-scores.
        This method wraps the FedReLa module.
        ...
        """
        # Call the FedReLa module function
        relabel_info = fedrela_relabel_local_data(
            g_head, net, net_id, args, relabel_thre, 
            self.ldr_train, class_num, read, store, method
        )
    ...
    def update_weights(self, net, seed, net_glob, epoch, mu=1, lr=None):
        ...
        for iter in range(epoch):
            ...
            for batch_idx, (index, images, labels) in enumerate(self.ldr_train):
    ...
    def update_weights_gaux(self, net, g_head, g_aux, l_head, epoch, args, mu=1, lr=None, loss_switch=None, isbest = False, net_id = 0, rnd = 0):
        net.train()
        # train and update
        ...
        if loss_switch == "focus_loss":
            criterion_l = focus_loss(num_classes=args.num_classes)

        args.relabel_cal = args.relabel_start
        if rnd >= args.relabel_cal:
            if rnd == args.relabel_start:
                ...
                read = 1
                store = 0
            relabel_info = self.relabel_local_data(copy.deepcopy(g_head),copy.deepcopy(net),net_id,args,
                                        relabel_thre = args.thre, class_num = args.num_classes, read =read, store= store, method='loge')
        epoch_loss = []
        fflag = 0
        for iter in range(epoch):
            batch_loss = []
            ...
            for batch_idx, (index, images, labels) in enumerate(self.ldr_train):
                ...
                if rnd >= args.relabel_start:
                    for j, value in enumerate(index):
                        if relabel_info and str(index[j]) in relabel_info['relabel_flag'].keys():
                            fflag = fflag + 1
                            #print(str(index[j]),labels[j],'->', relabel_info['relabel_flag'][str(index[j])])
                            labels[j] = relabel_info['relabel_flag'][str(index[j])]
                labels = labels.long()
            ...
            epoch_loss.append(sum(batch_loss)/len(batch_loss))
        if rnd > args.relabel_cal:
            print('net_id:',net_id, ' has relabeled:', fflag)
    ...
    def update_weights_etf(self, net, g_head, g_aux, l_head, epoch,args, mu=1, lr=None, loss_switch=None, isbest = False, net_id = 0, rnd = 0):
        net.train()
        # train and update
        ...
        if rnd >= args.relabel_cal:
            if rnd == args.relabel_start:
                ...
                store = 0
            relabel_info = self.relabel_local_data(copy.deepcopy(g_head),copy.deepcopy(net),net_id,args,
                                        relabel_thre = args.thre, class_num = args.num_classes, read =read, store= store, method='etf')
...
## gain personalzied l_head
    def update_weights_norm_init(self, net, net_id, args, g_aux, g_head, l_head, seed, net_glob, epoch, mu=1, lr=None):

        net.train()
        # train and update
        #relabel_info = get_noise_index_refined(self.ldr_train,g_head,net,net_id,args,
        #                                thre = args.thre, class_num = 10, read =1, store= 0)
...
def globaltest_head(net, g_head, test_dataset, args, dataset_class=None, head_switch=True):
    global_test_distribution = dataset_class.global_test_distribution
    three_shot_dict, _ = shot_split(global_test_distribution, threshold_3shot=[75, 95])
    correct_3shot = {"head": 0, "middle": 0, "tail": 0}
    total_3shot = {"head": 0, "middle": 0, "tail": 0}
    ...
    acc_3shot_global["tail"] = correct_3shot["tail"] / \
        (total_3shot["tail"] + 1e-10)
    return acc, acc_3shot_global
def localtest_head(net, g_head, l_head, test_dataset, dataset_class, idxs, user_id):
...
def globaltest_calibra(net, g_aux, test_dataset, args, dataset_class=None, head_switch=True, ji = 1):
    global_test_distribution = dataset_class.global_test_distribution
    print('global_test_distribution', global_test_distribution)
    three_shot_dict, _ = shot_split(global_test_distribution, threshold_3shot=[75, 95])
    if args.imbtype == 'step:':
        if args.classnum == 10:
            three_shot_dict = {"head": [1,2,3,4], "middle": [0], "tail": [5,6,7,8,9]}
        else:
            three_shot_dict = {"head": [i for i in list(range(1,50))], "middle": [0], "tail": [i for i in list(range(50,100))]}
