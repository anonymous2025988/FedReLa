...
global_test_head = 'g_head'
internal_frozen = False

etf_layer = True
loss_switch = "dot_reg_loss"
...
if __name__ == '__main__':
    ...
    args.relabel_cal = 100
    args.relabel_start = 200
    args.rounds = 220
    ...
    if len(load_switch) > 3:
        rndstart = args.relabel_start
        load_dir = "./output/cifar10/"
        model_load = torch.load(load_dir + args.load +"model_" + "best" + ".pth").to(args.device)
        w_glob_load = model_load.state_dict()
    ...
    for rnd in tqdm(range(rndstart, args.rounds)):
        ...
        for client_id in range(args.num_users):
            ...
            w_local, g_aux_temp, l_heads[client_id], loss_local = local.update_weights_etf(net=copy.deepcopy(model).to(args.device), g_head = copy.deepcopy(g_head).to(args.device), g_aux = copy.deepcopy(g_aux).to(args.device), l_head = l_heads[client_id], epoch=args.local_ep, loss_switch = loss_switch, args=args, isbest=is_best, net_id=client_id, rnd=rnd)
