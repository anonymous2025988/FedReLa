...
if __name__ == '__main__':
    ...
    if args.relabeltest:
        model = torch.load(load_dir + args.id +"model_bestN.pth").to(args.device)
        g_head = torch.load(load_dir + args.id +"g_head_bestN.pth").to(args.device)
        g_aux = torch.load(load_dir + args.id +"g_aux_bestN.pth").to(args.device)
        print('relabel model loaded:', load_dir + args.id +"model_bestN.pth")
        l_heads = []
        for i in range(args.num_users):
            l_heads.append(torch.load(load_dir +  args.id +"l_head_" + str(i) + "bestN.pth").to(args.device))
    else:
        model = torch.load(load_dir + args.id +"model_best.pth").to(args.device)
        g_head = torch.load(load_dir + args.id +"g_head_best.pth").to(args.device)
        g_aux = torch.load(load_dir + args.id +"g_aux_best.pth").to(args.device)
        l_heads = []
        print('Clean model loaded:', load_dir + args.id +"model_best.pth")
        for i in range(args.num_users):
            l_heads.append(torch.load(load_dir +  args.id +"l_head_" + str(i) + "best.pth").to(args.device))
    ...
    if args.relabeltest:
        ...
        fac = [1]
    for ji in fac:
        acc_s2, global_3shot_acc = globaltest_calibra(copy.deepcopy(model).to(args.device), copy.deepcopy(g_aux).to(args.device), dataset_test, args, dataset_class = datasetObj, ji=ji)
        ...
        # Local performance evaluation is disabled
        # # local test 
        # acc_list = []
        # f1_macro_list = []
        # f1_weighted_list = []
        ...
        print(args.id + 'round %d, global test acc  %.4f \n'%(rnd, acc_s2))
